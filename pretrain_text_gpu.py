...
def get_raw_model(model: nn.Module) -> nn.Module:
    """Helper to unwrap DDP model to access custom methods."""
...
def compute_moe_metrics(model: nn.Module) -> Dict[str, float]:
    ...
    # Note: If wrapped in DDP, model.modules() still recurses correctly
...
def _device(local_rank: Optional[int] = None) -> torch.device:
...
def create_model(config: PretrainConfig, metadata, device: torch.device, world_size: int, local_rank: int):
    model_cfg = dict(
    ...
    if config.load_checkpoint:
        ...
        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            ...
            state_dict = checkpoint  # fallback for raw SD
    
        # Extract architecture config from checkpoint metadata
        ckpt_config = checkpoint.get("config", {})
        arch_config = ckpt_config.get("arch", {})
    
        # Backwards compatibility: rename L_level.layers -> flat_layers
        ...
        print("[checkpoint] Model weights loaded successfully.")
    ...
    # Handle sparse embeddings optimizer separately if needed
    raw_model = model # Temporary pointer to raw model before DDP wrap logic implies we access attributes
    
    if hasattr(raw_model, "model") and hasattr(raw_model.model, "puzzle_emb"):
        puzzle_emb = getattr(raw_model.model, "puzzle_emb", None)
    ...
    if dist.is_available() and dist.is_initialized():
        # find_unused_parameters=True is usually required for MoE / conditional computation models
...
def init_train_state(config: PretrainConfig, metadata, device: torch.device, world_size: int, local_rank: int) -> TrainState:
    total_sequences = (
    ...
    model, optimizers, optimizer_lrs = create_model(config, metadata, device, world_size, local_rank)
...
def save_checkpoint(config: PretrainConfig, state: TrainState, rank: int, config_dict: Dict[str, Any]) -> None:
    ...
    # Save the underlying model state dict (unwrap DDP)
    raw_model = get_raw_model(state.model)
...
def train(config: PretrainConfig, device: torch.device, rank: int, world_size: int, local_rank: int):
    ...
    # Unwrap for logging/checkpointing, but pass DDP model to setup_wandb for param counting
    raw_model = get_raw_model(state.model)
    ...
    for step_idx in range(state.total_steps):
        ...
        if state.carry is None:
            # Must access initial_carry from underlying model if DDP wrapped
            state.carry = raw_model.initial_carry(batch)  # type: ignore[call-arg]
        ...
        # Access modules via state.model (DDP handles .modules() recursion correctly)
        with torch.no_grad():
...
def main(cfg: DictConfig) -> None: 
    # Initialize Distributed Group via torchrun env vars
    ...
    # Set device specifically to local rank
    torch.cuda.set_device(local_rank)
    device = _device(local_rank)
    
    print(f'Rank:{rank}, World Size:{world_size}, Local Rank:{local_rank}, Device: {device}') 
    raw_config = PretrainConfig(**OmegaConf.to_container(cfg, resolve=True)) 
    _seed_everything(raw_config.seed + rank) 
    
    try: 
        train(raw_config, device, rank, world_size, local_rank) 
    finally: 
        if dist.is_available() and dist.is_initialized(): 
            dist.barrier()
